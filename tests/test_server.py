...
async def test_tools_list_within_budget() -> None:
    ...
    swapped "the screen list" for "the match list", taking 19 chars back off. The zero on the core
    lane is the informative half: every edit sits on ``find_device``, which is display-gated, so
    the core lane cannot move, and it is measured rather than assumed not to have. The ``115 KB``
    guide claim edited in ``server.py`` rides the wire on ``get_guide``, a core tool, and moved
    neither lane because it replaced a figure of the same width.
    ...
    the document is served on request rather than advertised.
    ...
    the guide for the layer that is, 65_402 / 78_207 -> 65_692 / 78_497 (+290 on both); then the
    post-build review sent the alarm paragraph back, because it asserted a condition string nobody
    here had measured and said nothing about the empty ``alarm.message`` the client code calls
    common, 65_692 / 78_497 -> 66_062 / 78_867 (+370 on both). Every char of all three is
    description text. Both lanes move by the same amount because every tool touched is CORE, the
    mirror image of the five display-gated edits below.
