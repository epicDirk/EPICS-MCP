...
def test_check_with_a_database_compares_the_coverage_pins_too(
    ...
    monkeypatch.setattr(
        guard_audit, "load_coverage_map", lambda _path: {("olog_client.py", 182): {covering}}
    )
...
def test_a_coverage_context_is_matched_by_file_and_function(
    ...
    for context, expected in ((mine, counted - 1), (impostor, counted)):
        monkeypatch.setattr(
            guard_audit,
            "load_coverage_map",
            lambda _path, hit=context: {("olog_client.py", 182): {hit}},
        )
...
def test_the_candidate_list_is_pinned_by_its_members_not_only_its_length(
    ...
    monkeypatch.setattr(
        guard_audit, "load_coverage_map", lambda _path: {("olog_client.py", 182): set()}
    )
