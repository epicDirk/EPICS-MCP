...
* a STAND-IN test tree under ``tmp_path``, swapped in through ``guard_audit._TESTS``, with one
  ...
  very file as part of the real tree, and a ``setattr(..., "DemoClient", ...)`` written as code in a
  ``test_`` body here would move the real population. Each tree is written completely before the
...
def test_recursion_terminates_and_a_recursive_installer_still_counts(
    ...
    """Measured on this tree, ten test helpers call themselves; the walk remembers its visits."""
    population = _population(
        tmp_path,
        monkeypatch,
        {
...
def test_fixtures_are_followed_by_request_and_by_chain_but_helper_parameters_are_not_requests(
    ...
    a value its caller passes, measured on this tree in eleven live helpers named like a fixture.
...
def test_autouse_scope_follows_pytest_module_class_and_conftest(
    ...
    not reach the module test beside the class (measured: five class-level autouse fixtures in
    ``test_cli_init.py``); a ``conftest.py`` autouse fixture reaches every module beside it."""
    population = _population(
        tmp_path,
        monkeypatch,
        {
