...
|------|---------|-----------|
| `find_channels` | ChannelFinder — which IOC/host serves a PV + tags/properties; optional property/tag filters (`has_properties`/`lacks_properties` incl. `prop!=*` "lacks", `not_property_values`, `has_tags`/`lacks_tags`) and `count_only` for the exact match count via `/count`. Property filtering is gated to the DS-privacy safe-property allowlist (a redacted property like `accessGroup` is refused) | `EPICS_MCP_CHANNELFINDER_URL` |
