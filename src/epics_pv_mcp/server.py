...
def build_instructions(display_tools_available: bool) -> str:
    ...
    return (
        ...
        "server: PV searches follow the standard EPICS env (EPICS_PVA_ADDR_LIST / "
        "EPICS_CA_ADDR_LIST, EPICS_PVA_NAME_SERVERS, and the *_AUTO_ADDR_LIST auto search, "
        "which EPICS defaults to ON — even a null environment broadcasts into the local "
        "subnets), and a deployment may well have pointed the env at a real facility — so do NOT "
