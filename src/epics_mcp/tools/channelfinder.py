...
(:func:`epics_mcp.services.checkers.query_channels`) so :mod:`~epics_mcp.services.diagnose`
and :mod:`~epics_mcp.tools.find_device` reuse it without an upward ``services → tools`` import
...
async def _find_channels(
    ...
    *,
    ...
    Thin MCP adapter over :func:`epics_mcp.services.checkers.query_channels`; forwards the MA-2
...
async def _list_channel_vocabulary(timeout: float = 5.0) -> ChannelVocabularyResult:
    ...
    Thin MCP adapter over :func:`epics_mcp.services.checkers.query_channel_vocabulary`;
