...
def setup_epics_mcp(*, presets: Mapping[str, Preset]) -> str:
    ...
    return (
        ...
        "Two things NEVER belong in what you tell me to run, however convenient they look:\n"
        "   - EPICS_MCP_ALLOW_PV_WRITE or EPICS_MCP_ALLOW_OLOG_WRITE. Arming a write gate is a "
        "decision I make knowingly, never a step in a setup, and the check below cannot even see "
        "it. If I ask for writes, send me to docs/safety.md first.\n"
        ...
        "encoding my shell prefers, and in Windows PowerShell that is bytes a strict JSON parser "
        "rejects. If the file already exists the command refuses instead of overwriting, because "
