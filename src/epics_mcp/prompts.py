...
def setup_epics_mcp(*, presets: Mapping[str, Preset]) -> str:
    ...
    return (
        ...
        "reported by the epics://config resource, which the client keeps: that resource strips "
        "a userinfo before printing, error messages do too, and the server log does NOT, so a "
        "credential still has no business being there. Where a "
        ...
        "(what a running server ended up with is the rest_tls block in epics://health, which "
        "resolves that precedence and says whether any plane speaks https at all); "
        ...
        "back with me, and treat these four as findings rather than noise:\n"
        ...
        "   - '»' throttled: nothing was sent, EPICS_MCP_READ_RATE_LIMIT refused it\n"
