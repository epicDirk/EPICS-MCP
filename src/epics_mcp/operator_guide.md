...
| `»` | `throttled` | no request left this process for this plane: this command's OWN read throttle (`EPICS_MCP_READ_RATE_LIMIT`) refused the probe, so nothing here describes the service and every other status would be a claim about something that was never contacted. Not a verdict about the deployment but about this run's budget, so it is exit `3` (INCONCLUSIVE) like `identity_probe_failed`, and for the same reason: not a failure, and not a silent all-clear either. Size the limit for a WHOLE run, see the read-throttle note earlier in this guide |
