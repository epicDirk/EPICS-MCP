...
Kept out of the transport client (:mod:`epics_mcp.services.olog_client`) and the service
orchestrator (:mod:`epics_mcp.services.checkers`) so the byte handling is unit-testable in
...
from epics_mcp.paths import resolve_new_file_path, resolve_user_path
from epics_mcp.services.olog_client import AttachmentUpload
...
def plan_attachments(
    ...
    :func:`~epics_mcp.paths.resolve_user_path` (kind ``file``, it must exist) and sized by
...
def write_download(output_path: str, content: bytes, *, label: str = "output_path") -> str:
    ...
    Uses :func:`~epics_mcp.paths.resolve_new_file_path` so the ``EPICS_MCP_ALLOWED_ROOTS``
    boundary
