...
Structure mirrors :mod:`epics_mcp.services.naming_client` (Session + Retry on
...
# :func:`~epics_mcp.services.redact.project_allowlist`, so an unknown/new person-bearing property
...
def _resolve_allowlist(value: str | None, default: frozenset[str]) -> frozenset[str]:
    ...
    Three-way (see :class:`~epics_mcp.config.EpicsConfig`): ``None`` (unset) → the built-in
