...
| Tool | Description |
|------|-------------|
| `get_guide` | The operational cookbook as a tool: service planes, recipes, error signatures. `topic` serves one named part verbatim instead of the whole document. The keys PARTITION the guide, so a section key serves that section's own text and the subsections under it are keys of their own (`tools` is the tool inventory, not the whole palette; `errors` is the signature section's lead, and the signatures themselves are the `err-` keys). **How many keys there are, how large the document is, and how small its largest single part, are all answered where they are measured and not restated here**: the key list is derived from `TOPICS` into the argument description, and the sizes are promised at the tool itself and guarded there (`get_guide`'s own description in `src/epics_mcp/server.py`, held by `tests/test_guide_tool.py::test_the_rounded_size_claims_still_hold`): this table names neither figure, because a size restated on an unguarded page is a copy that ages. An unknown topic is refused by name rather than guessed. It contacts nothing, so it is safe as the first call of a session. The same text is the `epics://guide` resource below; a resource is application-controlled, which is why the tool exists beside it |
...
| `crossplane_check` | PV provenance: display PVs ↔ e3 IOC `st.cmd` (+ optional `.db`) ↔ Naming (read-only). The linked files are operator-facing, not screens only: `displays_linked` is the whole list, `screens_linked` and `trends_linked` split it by kind |
| `coverage_audit` | Cross-plane coverage matrix: delivered PVs (ChannelFinder) ↔ displays ↔ archive ↔ alarm; blind spots + critical-uncovered. A `.plt` Data Browser trend is not a screen: `has_display` answers about screens, `on_trend` beside it, and a PV reachable only through a trend is named in `trend_only` and counts as a `cf_only` blind-spot (`cf_trend_only` is that subset) |
| `find_device` | Where a device is shown, its live channel values (capped), and the serving IOC. A match is an operator screen OR a Data Browser trend opened by a button, said on `screens[].node_kind` and split by `display_count` / `trend_count`. The live cap does not shorten the match list; the inventory walk's own two caps can, and each fires a `notes` entry naming the count |
