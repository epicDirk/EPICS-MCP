...
| `EPICS_MCP_MAX_WRITE_STEP` | `0.0` | Largest DISTANCE one write may move a value, in the record's own units (0 = disabled, opt-in). The second post-admission refusal beside the drive-limit check, and it catches what that one cannot: a wrong setpoint INSIDE `[DRVL, DRVH]` is in range and still a jump nobody authorised. Refused with `PV_WRITE_STEP_TOO_LARGE` before the put, audit line `STEP_DENY`. Deliberately not derived from the drive limits: a permitted step is a property of how the plant is driven, not of the record, which is why `control_t` has no field for it. An enum record, a missing or non-finite current value, and a non-numeric written value are not step-checkable and the write proceeds with `step_note` saying so (≥ 0) |
